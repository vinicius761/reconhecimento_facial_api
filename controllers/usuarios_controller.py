...
async def processar_cadastro_usuario(
    ...
    try:
        ...
        resultado = db.execute(
            text("""
                INSERT INTO usuarios (nome, cpf, email, foto_path, embedding, nivel_acesso)
                VALUES (:nome, :cpf, :email, :foto_path, CAST(:embedding AS jsonb), :nivel_accesso)
                RETURNING id
            """),
            {
                "nome": nome,
                ...
                "nivel_accesso": nivel_acesso.upper(),  # Garante em caixa alta
            },
        )
